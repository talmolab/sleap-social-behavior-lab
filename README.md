...
```bash
uv run marimo run notebooks/04_collapsing_to_a_manifold.py
```
...
- `answer_key.csv` — ground-truth categories for the training events (a grading aid / fallback for
  the decoding step in notebook 06).
...
The Week-2 neural notebooks (07–08) were remade from the original EDGE Colab sources kept in
[`2025/`](2025/) (the NEU 457 lineage), preserved for provenance. Their display data is precomputed
offline by `tools/build_nb07_assets.py` / `tools/build_nb08_assets.py` (which read the original public
datasets) into the committed `data/nb07_assets.npz` / `data/nb08_assets.npz`, so students never
download from external hosts.
