...
def _(mo):
    mo.md(
        r"""
        # 06 · Dynamics and decoding

        ## The question so far, and the question now

        In notebook 4 we collapsed each interaction down to a small set of numbers and built a **map**
        of behavior — a picture in which similar interactions sit near each other. In notebook 5 we
        practiced reading such results honestly: we saw how the wrong unit of analysis, a leaky
        cross-validation split, or a large sample size can each make an effect look realer, or larger,
        than it is. We carry that same discipline into this notebook.

        We have described behavior. This notebook turns the description into two usable tools, and asks
        two matched questions.

        1. **Does behavior have structure in *time*?** Every event so far has been treated as a still
           picture — one clip, summarized into features. But animals do not jump between behaviors at
           random. If a cage is resting now, is it more likely to be resting a moment later, or is each
           moment an independent coin flip? And across the 24-hour day, when are these mice active? We
           are looking for two things a still picture cannot show: **memory** (does now predict next?)
           and **rhythm** (does activity rise and fall on a daily clock?).

        2. **Can we *predict* a behavior from the features?** We have been hand-labeling aggression by
           eye. If the 19 features carry enough information, a model should be able to read aggression
           out of them automatically — a **decoder**. The honest test is not whether it works on the
           clips it learned from, but whether it works on animals, and even a whole second experiment,
           that it never saw.
...
@app.cell
def _(cu, der, ev):
    # We resolve the running example by its stable event_key, never a raw integer index (the bundle is
    # periodically rebuilt and re-ordered, so an integer would drift to a different event). On the
    # current two-cohort bundle this is a fully-tracked interaction from cage 110 (a female cage). Its
    # ranks are [Sub, Mid, Dom] -> approacher=Sub (green), approachee=Mid (blue), bystander=Dom (red).
    # Its registry category is an ambiguous near-miss that an earlier model flagged as aggression but a
    # human scored as NOT aggression (agg_label = 0). That ambiguity is useful later, when we watch the
    # decoder score it near its decision boundary.
...
def _(mo):
    mo.md(
        ...
        - **Inputs** — `speed` (T, 3), the movement speed of each mouse, and `centroids` (T, 3, 2), the
          body-center position of each mouse. Optional `s_move` / `d_close` override the thresholds.
        - **Output** — `state_seq` (T,), one integer per frame, plus the list of state names.

        It applies two thresholds that are chosen *from the data itself*, by percentile:

        - **rest (0)** — average mouse speed below the 40th percentile of speed.
...
def _(mo):
    mo.md(
        r"""
        ## A.2b · How arbitrary are the state definitions? (stress-testing the grammar)

        Here is an uncomfortable fact worth confronting head-on: the three states are **defined by a
        threshold we picked**. "Rest" means "below the 40th percentile of speed"; "huddle" means "closer
        than the 25th-percentile pair distance". Those percentiles are a **convention**, not a law of
        nature — nothing physical happens at exactly the 40th percentile. A skeptic could reasonably ask:
        *did you cherry-pick the cuts that make the grammar look structured?*

        The honest response is to **vary the cuts and see whether the conclusion survives** — exactly the
        move notebook 5 pressed on us. Below, choose the speed percentile (30 / 40 / 50) and the distance
        percentile (15 / 25 / 35) and watch two things: (1) the **state fractions shift** — a higher
        speed cut relabels more frames as "rest", so the cut is genuinely arbitrary; but (2) the
        **entropy stays far below the memoryless line and stickiness stays far above chance** at every
        setting. The *number* changes; the *finding* — behavior carries memory — does not. That
        robustness is what earns the grammar our trust, not the specific default.
        """
    )
    return
...
@app.cell
def _(mo):
    disc_speed_pct = mo.ui.dropdown(options={"30th": 30, "40th (default)": 40, "50th": 50},
                                    value="40th (default)", label="rest speed cut (percentile)")
    disc_dist_pct = mo.ui.dropdown(options={"15th": 15, "25th (default)": 25, "35th": 35},
                                   value="25th (default)", label="huddle distance cut (percentile)")
    return disc_dist_pct, disc_speed_pct


@app.cell
def _(cu, disc_dist_pct, disc_speed_pct, go, grammar, mo, np, t15):
    # Recompute the cage-15 grammar at the chosen percentile cuts (pure recompute; the shuffle null is
    # the pinned one from the default grammar, drawn as a reference band).
    _sp = np.asarray(t15["speed"], float); _cen = np.asarray(t15["centroids"], float)
    _mean_speed = np.nanmean(_sp, axis=1)
    _dif = _cen[:, :, None, :] - _cen[:, None, :, :]
    _iu = np.triu_indices(3, k=1)
    _min_pair = np.nanmin(np.linalg.norm(_dif, axis=3)[:, _iu[0], _iu[1]], axis=1)
    _sm = np.nanpercentile(_mean_speed, disc_speed_pct.value)
    _dc = np.nanpercentile(_min_pair, disc_dist_pct.value)
    _seq, _names = cu.discretize_states(_sp, _cen, s_move=_sm, d_close=_dc)
    _T = cu.transition_matrix(_seq, 3)
    _H = cu.transition_entropy(_T)
    _self = float(np.mean(np.diag(_T)))
    _frac = np.bincount(_seq, minlength=3) / len(_seq)
    _null_H_mean = float(grammar["15"]["null_H"].mean())

    _from_sub = go.Figure()
    # entropy panel: observed vs memoryless line vs shuffle null
    _from_sub.add_bar(x=["entropy (bits)"], y=[_H], marker_color="#1b9e77", width=0.4,
                      name="observed", showlegend=False)
    _from_sub.add_hline(y=1.585, line=dict(color="#bbb", dash="dot"),
                        annotation_text="memoryless log₂3 = 1.585", annotation_position="top left")
    _from_sub.add_hline(y=_null_H_mean, line=dict(color="#e45756", dash="dash"),
                        annotation_text=f"shuffle null ≈ {_null_H_mean:.2f}", annotation_position="bottom left")
    _from_sub.update_layout(template="plotly_white", height=300,
                            title=f"Cage-15 entropy at speed-{disc_speed_pct.value} / dist-{disc_dist_pct.value} cuts",
                            yaxis_title="bits", yaxis_range=[0, 1.7],
                            margin=dict(l=10, r=10, t=44, b=10))

    _fr = go.Figure(go.Bar(x=_names, y=_frac, marker_color=["#9e9e9e", "#ff7f0e", "#6a3d9a"]))
    _fr.update_layout(template="plotly_white", height=300, title="State fractions (these DO move)",
                      yaxis_title="fraction of time", yaxis_range=[0, 0.7],
                      margin=dict(l=10, r=10, t=44, b=10))
    _fr.update_xaxes(showgrid=False)

    mo.vstack([
        mo.hstack([disc_speed_pct, disc_dist_pct]),
        mo.hstack([_from_sub, _fr]),
        mo.md(f"*At these cuts: entropy **{_H:.3f} bits** (still far below the memoryless 1.585 and the "
              f"shuffle null ≈ {_null_H_mean:.2f}), stickiness **{_self:.3f}** (far above the ~0.33 of "
              f"three equally likely states), state fractions **{np.round(_frac, 2).tolist()}**. Move the "
              f"cuts and the fractions swing, but the memory verdict never flips. The default cuts are a "
              f"convention; the conclusion is robust to it.*")
    ])
    return
...
def _(mo):
    mo.md(
        ...
        **The function.** `cu.transition_matrix(state_seq, n_states)`:
        ...
        Pick a cage below to see its matrix; the caption reports how far it sits from the example
        cage 15.
        """
    )
    return
...
def _(STATE_NAMES, cage_pick, go, grammar, mo, np):
    ...
    _fig.update_layout(template="plotly_white", height=420,
                       ...
                       margin=dict(l=10, r=10, t=44, b=10))
    _fig.update_yaxes(autorange="reversed", showgrid=False)
    _fig.update_xaxes(showgrid=False)
    _delta = float(np.abs(_T - _T15).max())
    _cmp = ("this **is** cage 15, the reference." if _c == "15"
            else f"largest cell-by-cell difference from cage 15: **{_delta:.2f}** — the three cages "
                 f"share nearly the same grammar despite different animals and sexes.")
    mo.vstack([cage_pick, _fig,
               mo.md(f"*The diagonal ({np.round(np.diag(_T),2).tolist()}) is the largest part of each "
                     ...
                     f"three equally likely states would be 1/3 ≈ 0.33). And {_cmp}*")])
    return
...
def _(mo):
    mo.md(
        ...
        the next state, take a step, and repeat many times. Tally how often the walker lands in each
        state. That tally *is* the stationary distribution.
...
@app.cell
def _(mo):
    walk_steps = mo.ui.slider(20, 3000, value=300, step=20,
                              label="walker steps (Monte-Carlo sample size)",
                              debounce=True, full_width=True)
    return (walk_steps,)
...
def _(STATE_COLORS, STATE_NAMES, cu, go, grammar, mo, np, walk_steps):
    ...
    _fig.update_layout(template="plotly_white", height=380,
                       yaxis_title="fraction of time", margin=dict(l=10, r=10, t=70, b=10))
    _fig.update_xaxes(showgrid=False)
    cu.apply_house_style(_fig, title="Cage-15 stationary distribution — walker vs. truth",
                         legend="below")
    _err = float(np.abs(_pi_sim - _pi_true).max())
    mo.vstack([walk_steps, _fig,
               mo.md(f"*Largest gap between walker and truth: **{_err:.3f}**. At a few hundred steps this "
                     f"gap is still a few percent and jumps around as you nudge the slider (that jitter is "
                     f"Monte-Carlo noise); by a few thousand steps it settles near zero. A faster exact "
                     f"shortcut using an eigenvector is in the note below.*")])
    return
...
def _(mo):
    mo.md(
        ...
        null draws, not a single summary bar, is the honest way to judge a gap — the same principle from
        notebook 5's permutation tests.
        """
    )
    return
...
@app.cell
def _(CAGE_COLORS, cu, go, grammar, mo, np):
    ...
    for _c in _cages:
        _fig.add_trace(go.Scatter(x=["shuffle null"], y=[grammar[_c]["self"]], mode="markers",
                       ...
                       row=1, col=2)
    _fig.update_layout(template="plotly_white", height=440, margin=dict(l=10, r=10, t=64, b=10))
    _fig.update_yaxes(range=[0, 1.7], row=1, col=1, title="bits", showgrid=False)
    _fig.update_yaxes(range=[0, 1.0], row=1, col=2, title="P(stay)", showgrid=False)
    ...
    mo.vstack([_fig, mo.md(
        ...
        f"time. Not one of the 40 shuffles per cage comes anywhere near the observed value — the "
        f"grammar carries real memory.*")])
    return
...
def _(mo):
    mo.md(
        ...
        - **Output** — a curve of mean movement speed binned into (here) 60-minute slots.
        ...
        **Read this as a description, not a circadian law.** Three cages cannot support a population
        claim about circadian rhythm — this is exactly the pseudoreplication caution from notebook 5. We
        are describing *these* recordings, honestly.
        """
    )
    return
...
@app.cell
def _(CAGE_COLORS, clocks, cu, ex_tod, go, grammar, mo):
    _fig = go.Figure()
    ...
    for _c in ["15", "10", "13"]:
        ...
        _fig.add_scatter(x=_ck["centers"], y=_ck["curve"], mode="lines", line=dict(color=_col, width=2.5),
    ...
    _fig.update_xaxes(range=[0, 24], dtick=3, showgrid=False)
    cu.apply_house_style(_fig, title="Activity clock — mean speed by time of day (60-min bins)",
                         legend="below")
    mo.vstack([_fig, mo.md(
...
def _(mo):
    mo.md(
        ...
        **Your task (one line to fill in).** In the cell below, replace the `# TODO` line with
...
def _(np, t15):
    ...
    def student_tmat(seq, K):
        ...
        np.add.at(M, (seq[:-1], seq[1:]), 1)     # <-- the # TODO line (already filled with the answer)
...
def _(T_student, cu, grammar, np, seq15):
    # ---- graded self-check: exact match + robust gap + a permutation p-value vs the shuffle null ----
    ...
    # Permutation p-values (notebook-5 style): one-sided, "how many shuffles are at least as extreme?"
    # entropy is LOWER when there is memory; stickiness is HIGHER. (1+#extreme)/(1+n) never gives 0.
    n_null = len(_null_H)
    p_entropy = (1 + int(np.sum(_null_H <= H_obs))) / (1 + n_null)
    p_self = (1 + int(np.sum(_null_self >= self_obs))) / (1 + n_null)
    ...
    return (H_obs, bands_ok, entropy_gap, n_null, p_entropy, p_self, self_gap,
            self_obs, tier1_ok, tier2_ok)


@app.cell(hide_code=True)
def _(H_obs, bands_ok, entropy_gap, mo, n_null, p_entropy, p_self, self_gap,
      self_obs, tier1_ok, tier2_ok):
    ...
    mo.md(
        ...
        <b>Permutation p-values</b> ({n_null} shuffles) — entropy: <b>{int(round(p_entropy*(1+n_null)))-1}
        of {n_null}</b> shuffles reach an entropy this low → p = <b>{p_entropy:.3f}</b>; stickiness:
        <b>{int(round(p_self*(1+n_null)))-1} of {n_null}</b> shuffles reach a stickiness this high → p =
        <b>{p_self:.3f}</b>. (Reporting "0 of 40 shuffles beat the observed value" is the honest way to
        state a permutation p; with 40 draws the smallest p we can resolve is 1/41 ≈ 0.024.)<br>
...
def _(mo):
    mo.md(
        ...
        **A note on the tool we chose, and one we did not.** We modeled the dynamics as a *Markov chain*
        on three observed states. A richer alternative is a **hidden Markov model (HMM)**, which lets the
        states themselves be latent — inferred from the kinematics rather than defined by a percentile
        cut. An HMM can discover states we did not name and is the natural next step; we used the visible
        three-state chain because it is transparent (you can read every number off a table) and because
        A.2b showed the conclusion does not hinge on the exact cuts. When the goal shifts from "is there
        memory?" to "what are the true behavioral states?", reach for the HMM.
...
def _(mo):
    mo.md(
        ...
        about testing it honestly — with the cautions from notebook 5 front of mind.
...
def _(mo):
    mo.md(
        r"""
        The **confusion matrix** (left, below) shows the four outcomes at a 0.5 threshold, but
        **row-normalized** — each row divided by its total — so the color encodes *recall* (what fraction
        of each true class the decoder got right) rather than raw counts. That matters under class
        imbalance: with only 38% aggression, a count-colored matrix would light up the large "true-not"
        row and hide the error structure. The **calibration curve** (right) checks whether the
        probabilities mean what they say: for events scored around 0.7, did roughly 70% actually turn out
        to be aggression? A well-calibrated decoder follows the dotted diagonal.

        <div style="border-left:4px solid #888; padding:6px 12px; background:rgba(0,0,0,0.03);">
        <b>Reading this reliability curve.</b> Notice that the cam16 curve sits <b>above</b> the dotted
        diagonal at essentially every bin — events the decoder scores around 0.44 turn out to be
        aggression about 65% of the time, and even its most confident 0.92 bin lands at 0.98. The decoder
        is <b>systematically under-predicting</b> here. Why? It was trained where aggression made up only
        <b>32%</b> of events, so its scores are anchored to that <b>training base rate</b> of 0.32. cam16
        is a <i>higher</i>-aggression cage (base rate <b>0.385</b>), so the same scores correspond to more
        aggression than the model expects, and the observed fraction runs high. The pull is toward the
        <b>0.32 the model learned</b>, not toward cam16's own rate. If you only need the <i>ranking</i>
        (AUROC), this offset does not matter; if you need the <i>probability</i> to be literally true (e.g.
        to threshold at a target precision), you would re-calibrate on held-out data.
        </div>
        """
    )
    return
...
def _(cu, go, mo, np, res_ho, s_ho, yh):
    ...
    _fig1 = cu.confusion_fig(_cm, ["not agg", "agg"], normalize="row", title="Confusion @ 0.5 (row-normalized = recall)")
    ...
    _fig2.add_scatter(x=_mean, y=_frac, mode="lines+markers", line=dict(color="#4c78a8"),
                      showlegend=False)
    _fig2.update_layout(template="plotly_white", height=360, title="Calibration (reliability curve)",
                        xaxis_title="mean predicted P", yaxis_title="observed fraction",
                        margin=dict(l=10, r=10, t=44, b=10))
    _fig2.update_xaxes(showgrid=False); _fig2.update_yaxes(showgrid=False)
...
def _(mo):
    mo.md(
        ...
        that makes both errors zero, and the crossing point is where the two are balanced.
        """
    )
    return
...
def _(mo):
    mo.md(
        ...
        - **Model.** Is the linear logistic regression enough, or does a small **multi-layer perceptron
          (MLP)** — a neural network with a couple of hidden layers, which can carve curved (nonlinear)
...
def _(X, cu, der, featureset_btn, make_lr, mo, np, sweep, y):
    if not featureset_btn.value:
        ...
        _fig.add_hline(y=0.851, line=dict(color="#888", dash="dot"),
                       annotation_text="logistic · 19 features ≈ 0.851")
...
def _(mo):
    mo.md(
        ...
        the opposite of the naive expectation that "a neural network is more powerful". Two things are
        going on: the classes are close to **linearly separable** in this feature space, so the curved
        boundary an MLP can draw buys nothing; and an MLP has many more parameters to fit from only 2,499
        rows, so it is likely **undertuned / mildly overfit** at these default settings. The lesson is
        general: *more model capacity is not more accuracy* — match the model to the geometry of the
        problem, and let held-out CV, not intuition, decide.

        Second, **adding the cluster one-hots does not help** (if anything it hurts the MLP): the
        clusters are a coarser summary of the same 19 features, not new information. That is the whole
        point of the feature pipeline — the useful signal was already in the features. We keep the
        simpler, better logistic decoder from here on.

        <div style="border-left:4px solid #888; padding:6px 12px; background:rgba(0,0,0,0.03);">
        <b>A note that points to notebook 5: how we split the folds.</b> These folds use a
        <i>random</i> 5-fold split. That is safe <b>here</b> because our events are disconnected
...
def _(mo):
    mo.md(
        ...
        Because the dataset has **two independent cohorts** (recorded weeks apart, cohort-unique cages),
        we can run the strongest generalization test available short of a new experiment:
...
def _(X, cohort, cu, make_lr, np, res_ho, y):
    ...
    return cam16_auc, cv_folds, loco_scores
...
def _(cam16_auc, cu, cv_folds, go, loco_scores, mo, np):
    ...
    _fig = go.Figure()
    # CV group: jittered points at x=0 + a short mean-line segment spanning only that column.
    _rng = np.random.RandomState(0)
    _jx = 0 + (_rng.rand(len(cv_folds)) - 0.5) * 0.18
    _fig.add_scatter(x=_jx, y=cv_folds.astype(float), mode="markers",
                     marker=dict(color="#4c78a8", size=11, opacity=0.75,
                                 line=dict(width=1, color="white")),
                     name="CV fold", showlegend=False,
                     hovertemplate="fold AUROC %{y:.3f}<extra></extra>")
    _fig.add_scatter(x=[-0.22, 0.22], y=[cv_folds.mean(), cv_folds.mean()], mode="lines",
                     line=dict(color="#4c78a8", width=3), showlegend=False, hoverinfo="skip")
    # Single-value groups as diamonds (no mean line).
    _singles = [(1, cam16_auc, "#2ca02c")] + \
               [(2 + _i, loco_scores[t], "#7570b3") for _i, t in enumerate(_tags)]
    for _xpos, _v, _col in _singles:
        _fig.add_scatter(x=[_xpos], y=[_v], mode="markers",
                         marker=dict(color=_col, size=15, symbol="diamond",
                                     line=dict(width=1.5, color="black")),
                         showlegend=False,
                         hovertemplate=f"{_ticks[_xpos]}: %{{y:.3f}}<extra></extra>")
    ...
    _fig.update_layout(template="plotly_white", height=440,
                       ...
                       yaxis_title="ROC-AUC", margin=dict(l=10, r=10, t=50, b=10))
    _fig.update_xaxes(tickmode="array", tickvals=list(range(len(_ticks))), ticktext=_ticks,
                      range=[-0.6, len(_ticks) - 0.4], showgrid=False)
    _fig.update_yaxes(range=[0.45, 0.95], showgrid=False)
...
def _(mo):
    mo.md(
        r"""
        The features that hurt the most when shuffled are the ones you would hope for: the
        **approachee's speed** (`appe_speed_mean`), the two **facing** features
        (`appe_faces_appr`, `appr_faces_appe` — who is oriented toward whom), and how far the
        **bystander** sits (`bystander_dist_mean`). Aggression is a fast, face-to-face, two-body event,
        and the decoder reads exactly that geometry.

        <div style="border-left:4px solid #888; padding:6px 12px; background:rgba(0,0,0,0.03);">
        <b>Two cautions when reading a permutation-importance bar.</b> First, a low bar is <b>not</b>
        proof a feature is irrelevant. <code>closing_speed</code>, for instance, ranks near the
        <b>bottom (17th of 19)</b> here — yet closing speed is obviously relevant to aggression. The
        reason is <b>correlation</b>: closing speed is largely reconstructable from the speed and facing
        features, so shuffling it alone barely dents AUROC because the model reads the same information
        elsewhere. Permutation importance measures a feature's <i>unique, non-redundant</i> contribution,
        not its total relevance. Second, this is importance <i>for this fitted linear model on this
        cage</i>; a different model, or a different held-out cage, will reorder the middle of the list.
        Use it to sanity-check that the decoder reads sensible channels, not as a definitive ranking of
        which behaviors "cause" aggression.
        </div>
        """
    )
    return
...
def _(mo):
    mo.md(
        ...
        labels barely moves AUROC. Only once a large fraction is corrupted does performance collapse
        toward chance. (Notebook 5 goes deeper on why linear models tolerate label noise and where that
        robustness breaks.)
        """
    )
    return
...
@app.cell
def _(mo):
    noise_btn = mo.ui.run_button(label="▶ Simulate the label-noise ceiling (refits several models)")
    return (noise_btn,)
...
def _(X, Xh, cu, go, make_lr, mo, noise_btn, np, y, yh):
    if not noise_btn.value:
        ...
        _levels = [0.0, 0.05, 0.10, 0.20, 0.30, 0.40, 0.50]
        _aucs = []
        _rng = np.random.RandomState(0)
        for _p in _levels:
            _yn = y.copy()
        ...
        _fig.update_xaxes(showgrid=False); _fig.update_yaxes(range=[0.45, 0.9], showgrid=False)
        _out = mo.vstack([_fig, mo.md(
            f"*At 0% noise: AUROC **{_aucs[0]:.3f}**; at 10%: **{_aucs[2]:.3f}** (barely changed); at "
            f"30%: **{_aucs[4]:.3f}**; at 50% (labels random): **{_aucs[6]:.3f}**, near chance. The "
            f"decoder tolerates modest label noise and only collapses when the labels stop carrying "
            f"signal.*")])
    _out
    return
...
@app.cell
def _(X, Xh, cohort, cu, make_lr, y, yh):
    ...
    mo.md(
        ...
        **What we established this week.** Starting from raw SLEAP keypoints, we built a body-frame
        ...
        - **Behavior is decodable.** A simple logistic decoder reads aggression from the 19 features and,
          ...
          cross-dataset test — leave-one-cohort-out ≈ 0.825 / 0.859. A more complex model did not help;
          the signal was already in the features. We can now label behavior objectively and reproducibly,
          at scale, without a human scoring every frame.
        ...
        *what* an animal does, Week 2 turns to *how the brain produces it*: we move from tracking bodies
        ...
        neurons we must discover. How do we read the brain that produces the behavior we just learned to
        measure?
        """
    )
    return
